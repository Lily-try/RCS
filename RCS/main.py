...
def Community_Search(args,logger):
    ...
    nodes_feats, train, val, test, node_in_dim, n_nodes, edge_index, edge_index_aug, adj_matrix = load_datasets(args)
    ...
    for epoch in range(args.warm):
        ...
        for q, pos, comm in train:
            ...
            for param in embLearner.parameters():  # or model.mlp1.parameters()
    ...
    for epoch in range(args.epoch_n):
        ...
        for q, pos, comm in train:
            ...
            l2_lambda = 1e-4
            l2_reg = torch.tensor(0.0, device=h.device)
            for param in embLearner.parameters():
                if param.requires_grad:
                    l2_reg += torch.norm(param, p=2) ** 2
        ...
        if args.enable_refine:
            with torch.no_grad():
                ...
                edge_probs = torch.sigmoid(cos_sim)
                tau = args.sim_tau
